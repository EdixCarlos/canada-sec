...
@Component({
  selector: 'app-or-create',
  templateUrl: './or-create.component.html',
  styleUrls: ['./or-create.component.scss']
})
export class OrCreateComponent implements OnInit {
  ...
  public ticketType = TicketType[21];
  ...
  createForm() {
    this.formGroup = this.fb.group({
      ...
      id: ['s2810934', Validators.required],
      requestArea: [''],
      requestType: [''],
  ...
  disabledInputs() {
    if (this.isNewTicket || this.ticket.idStatus == 104) {
      ...
      this.formControls.requireType.disable(); 
    } else {
      this.formControls.requestedBy.disable();
      this.formControls.email.disable();
      this.formControls.id.disable();
  ...
  mapTicketToForm() {
    ...
    form.epmCode = dataJSON.requestTypeName;;
