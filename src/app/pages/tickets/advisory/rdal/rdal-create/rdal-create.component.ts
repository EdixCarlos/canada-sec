...
@Component({
  selector: 'app-rdal-create',
  templateUrl: './rdal-create.component.html',
  styleUrls: ['./rdal-create.component.scss']
})
export class RdalCreateComponent implements OnInit {
  ...
  public ticketType = TicketType[9];
  ...
  public analysisTypeList = [
    { id: 'Dynamic analysis (Webinspect)', description: 'Dynamic analysis (Webinspect)' },
    { id: 'Static analysis (Fortify)', description: 'Static analysis (Fortify)' },
    { id: 'Software composition (Blackduck)', description: 'Software composition (Blackduck)' }
  ];

  public vulnerabilitiesList = new BehaviorSubject(null);

  public vulnerabilitiesColumns: string[] = [
    "id",
    "vulnerabilityName",
    "severity",
    "location",
  ...
  get vulnerabilities() {
    return this.formGroup.get('vulnerabilities') as FormArray;
  }
  ...
  createVulnerabilities() {
    this.vulnerabilities.push(this.fb.group({
      id: [''],
  ...
  checkObservable() {
    ...
    this.activatedRoute.url.subscribe((url: UrlSegment[]) => {
      ...
      if (url[1].path === 'view') {
        ...
        this.formControls.requestType.setValue(this.ticketType);
  ...
  disabledInputs() {
    if (this.isNewTicket || this.ticket.idStatus == 104) {
      ...
      this.formControls.analysisType.disable();
  ...
  mapTicketToForm() {
    ...
    form.requestTypeName = dataJSON.requestType.description;
  ...
  deleteVulnerability(index: number) {
    this.vulnerabilities.removeAt(index);
    this.vulnerabilitiesList.next(this.vulnerabilities.value);
  }
