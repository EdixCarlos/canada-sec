...
import { projectFases } from 'src/app/core/model/projectFases';
import { SecurityModalComponent } from '../security-modal/security-modal.component';
import { PdfService } from 'src/app/core/_services/pdf.service';
import { DATAREPORT,REPORT } from 'src/app/core/model/advisor-tra';
...
@Component({
  selector: 'app-tra-create',
  templateUrl: './tra-create.component.html',
  styleUrls: ['./tra-create.component.scss']
})

export class TraCreateComponent implements OnInit {
  ...
  reportSCL: DATAREPORT[];
  userPolicies = [];
  fases: projectFases[] = [];
  listFases:string[]=["Planning","Development","Testing", "Production"];

  // For udf
  public FORM_NAME = "thr-ris-ass";
  ...
  public ticketType = TicketType[6];
  ...
  //PDF
  header = [['Ticket', 'Security Team', 'Domain', 'Security Control', 'NIST', 'Value', 'Additional Information']]
  ...
  constructor(
    ...
    public chkService: ChecklistService,
  ...
  createForm() {
    this.formGroup = this.fb.group({
      ...
      comments:[''],
      projectFase: ['',Validators.required],      
      additionalComments: [''],
      objFiles: []
  ...
  checkObservable() {
    ...
    this.activatedRoute.url.subscribe((url: UrlSegment[]) => {
      if (url[1].path === 'view') {
        ...
        // this.formControls.requireType.setValue(this.ticketType);
  ...
  disabledInputs() {
    if (this.isNewTicket || this.ticket.idStatus == 104) {
      ...
      this.formControls.comments.disable();
      this.formControls.projectFase.disable(); 
    }
  }
  ...
  openSecurityModal() {
    ...
    const materialDialogConf = modalConfig.buildModalConfig("100%", 
     data
    );
...
goToList() {
  ...
  getDataReport(){    
    this.chkService.getCheckList(this.idTicket).subscribe( data => {    
  ...
  generatePdf(){            
    this._pdf.generatePdf(this.header, this.dataSource);      
    this._pdf.outPut();          
  }
